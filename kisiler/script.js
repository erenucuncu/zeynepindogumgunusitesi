...
class Branch {
    ...
    getColor(index) {
        let base = ['#F482dd'];
        let greens = ['#F482dd']; //, '#5DC4A8', '#4BBD9E', '#3AB795', '#A7CCBA', '#91C0A9']
        let chooseFrom = index == 0 ? base : greens;
        return chooseFrom[Math.floor(Math.random() * greens.length)];
    }
}
class Flower {
...
class App {
    constructor(container) {
        ...
        this.height = 600;
        this.grid = 40;
        this.framePath = 'M -300 -175, L 300 -175, 300 175, -300 175 Z';
        this.images = [
            './images/1.png',
            './images/2.png',
            './images/3.png',
        ...
        this.selectedImage = 0; // Start from the first image
        ...
        document.getElementById('ileri').addEventListener('click', () => this.goToMaze());
    ...
    setImage() {
        ...
        this.audio.setAttribute('src', this.audioFiles[this.selectedImage]);
