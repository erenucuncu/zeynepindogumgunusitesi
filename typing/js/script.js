...
function initTyping() {
    ...
    if(charIndex < characters.length - 1 && timeLeft > 0) {
        ...
        clearInterval(timer);
        inpField.value = "";
        // Show the try again button when time ends
        tryAgainBtn.style.visibility = "visible";
        // Check if the user has finished the sentence correctly
...
function resetGame() {
    ...
    cpmTag.innerText = 0;
    // Hide the try again button when the game is reset
    tryAgainBtn.style.visibility = "hidden";
    // Hide the next page button when the game is reset
    nextPageBtn.style.visibility = "hidden";
}
...
nextPageBtn.addEventListener("click", () => {
    window.location.href = "../pazıl/index.html";
});
